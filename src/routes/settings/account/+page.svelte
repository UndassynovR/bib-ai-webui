<script lang="ts">
	...
	let isLocalAccount = $derived(
		user?.auth_type === "local" && !user?.is_guest,
	);
	...
	async function handlePasswordChange(e: Event) {
		...
		if (newPassword.length < 6) {
			passwordError =
				i18n.t("settings.passwordTooShort") ||
		...
		if (newPassword !== confirmPassword) {
			passwordError =
				i18n.t("settings.passwordsDoNotMatch") ||
	...
	async function handleDeleteAccount() {
		if (deleteConfirmText !== "DELETE") {
			deleteError =
				i18n.t("settings.typeDeleteToConfirm") ||
...
<svelte:head>
	<title>{i18n.t("settings.accountTitle")}</title>
</svelte:head>
			...
			<div class="info-item">
				...
				<div class="info-value">
					...
					{:else if user?.auth_type === "ldap"}
						{i18n.t("settings.universityAccount") ||
							"University Account"}
					{:else}
						{i18n.t("settings.personalAccount") ||
							"Personal Account"}
					{/if}
				</div>
			</div>
				...
				<div class="form-container">
					<div class="form-header">
						<div class="header-with-icon">
							...
							<h3>
								{i18n.t("settings.confirmDeletion") ||
					...
					<div class="warning-text">
						<p>
							<strong
								>{i18n.t("settings.deleteWarningTitle") ||
									"This action cannot be undone."}</strong
							>
						</p>
						<p>
							{i18n.t("settings.deleteWarningText") ||
								"All your data including chat history will be permanently deleted."}
						</p>
						<p class="confirm-instruction">
							{i18n.t("settings.typeDeleteInstruction") || "Type"}
							<strong>DELETE</strong>
							{i18n.t("settings.toConfirm") || "to confirm"}
						</p>
					</div>
					...
					>
						...
						<span>
							{isDeletingAccount
								...
								: i18n.t("settings.permanentlyDelete") ||
