<script lang="ts">
	...
	let libraryDbStatus = $state<"idle" | "testing" | "success" | "error">(
		"idle",
	);
	let libraryDbError = $state("");

	let libraryDbUrl = $derived(
		LIBRARY_DB_HOST &&
			LIBRARY_DB_NAME &&
			LIBRARY_DB_USER &&
			LIBRARY_DB_PASSWORD
	...
	async function testLibraryDb() {
		...
		try {
			const response = await fetch(
				"/api/admin/settings/test-library-db",
				{
					method: "POST",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({
...
<svelte:head>
	<title>{i18n.t("dashboard.settings.title")}</title>
</svelte:head>
	...
	<section class="settings-section">
		...
		<div class="form-group">
			...
			<input
				...
				type="password"
				bind:value={LDAP_ADMIN_PASSWORD}
				disabled={loading}
			/>
		</div>
		<div class="form-group">
	...
	<section class="settings-section">
		...
		<div class="form-group">
			...
			<input
				...
				type="password"
				bind:value={LIBRARY_DB_PASSWORD}
				disabled={loading}
			/>
		</div>
		{#if libraryDbUrl}
