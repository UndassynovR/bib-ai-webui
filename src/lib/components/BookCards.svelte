<script lang="ts">
	...
	function getAuthors(book: Book): string {
		return (
			...
			i18n.t("bookCards.authorNotSpecified")
		);
	}
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-3 p-2">
	{#each displayedBooks as book, index (book.DOC_ID || `book-${index}`)}
		<button onclick={() => handleBookClick(book)} class="book-card group">
			<!-- Book Info -->
...
{#if hasMoreBooks}
	<div class="show-more-container">
		...
		>
			{showAllBooks
				? i18n.t("bookCards.showLess")
				: i18n.t("bookCards.showAll", { count: books.length })}
		</button>
	</div>
{/if}
